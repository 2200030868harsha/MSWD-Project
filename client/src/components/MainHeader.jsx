import React, { useState } from 'react';
import { FaSearch } from 'react-icons/fa';
import { Link, useNavigate } from 'react-router-dom';
import { useSelector } from 'react-redux';
import { useEffect } from 'react';
import AdminNavbar from './AdminNavbar'; // Import AdminNavbar component

const MainHeader = () => {
  const { currentUser } = useSelector((state) => state.user);
  const [searchTerm, setSearchTerm] = useState('');
  const [userType, setUserType] = useState('user'); // Default to user type
  const navigate = useNavigate();

  const handleSubmit = (e) => {
    e.preventDefault();
    const urlParams = new URLSearchParams(window.location.search);
    urlParams.set('searchTerm', searchTerm);
    const searchQuery = urlParams.toString();
    navigate(`/search?${searchQuery}`);
  };

  useEffect(() => {
    const urlParams = new URLSearchParams(location.search);
    const searchTermFromUrl = urlParams.get('searchTerm');
    if (searchTermFromUrl) {
      setSearchTerm(searchTermFromUrl);
    }
  }, [location.search]);

  // Handle user type change
  const handleUserTypeChange = (type) => {
    setUserType(type);
  };

  // Handle sign in dropdown
  const handleSignIn = () => {
    // Open a dropdown or navigate to the sign-in page
    // For simplicity, let's navigate to the sign-in page for now
    navigate('/sign-in');
  };

  return (
    <>
      {userType === 'user' ? (
        <header className='bg-red-200 shadow-md'>
          <div className='flex justify-between items-center max-w-6xl mx-auto p-3'>
            <Link to='/'>
              <h1 className='font-bold text-sm sm:text-xl flex flex-wrap'>
                <span className='text-slate-500'>REMS</span>
                <span className='text-slate-700'>Project</span>
              </h1>
            </Link>
            <form
              onSubmit={handleSubmit}
              className='bg-slate-100 p-3 rounded-lg flex items-center'
            >
              <input
                type='text'
                placeholder='Search...'
                className='bg-transparent focus:outline-none w-24 sm:w-64'
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
              <button>
                <FaSearch className='text-slate-600' />
              </button>
            </form>
            <ul className='flex gap-4'>
              <Link to='/'>
                <li className='hidden sm:inline text-slate-700 hover:underline'>
                  Home
                </li>
              </Link>
              <Link to='/about'>
                <li className='hidden sm:inline text-slate-700 hover:underline'>
                  About
                </li>
              </Link>
              {/* Sign In Dropdown */}
              <div className='relative'>
                <button className='text-slate-700 hover:underline' onClick={handleSignIn}>
                  {currentUser ? (
                    <img
                      className='rounded-full h-7 w-7 object-cover'
                      src={currentUser.avatar}
                      alt='profile'
                    />
                  ) : (
                    'Sign In'
                  )}
                </button>
                {/* Dropdown menu */}
                {currentUser && (
                  <ul className='absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg z-10'>
                    <li className='py-1'>
                      <Link to='/profile' className='block px-4 py-2 text-gray-800 hover:bg-slate-200'>
                        Profile
                      </Link>
                    </li>
                    <li className='py-1'>
                      <button
                        onClick={() => handleUserTypeChange('admin')}
                        className='block px-4 py-2 text-gray-800 hover:bg-slate-200 w-full text-left'
                      >
                        Switch to Admin
                      </button>
                    </li>
                  </ul>
                )}
              </div>
            </ul>
          </div>
        </header>
      ) : (
        <AdminNavbar />
      )}
    </>
  );
};

export default MainHeader;
